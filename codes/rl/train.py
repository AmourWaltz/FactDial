...
def train(args, model, tokenizer):
    ...
    save_path = f'{args.exp_path}/{args.model_type}_batch{args.batch_size}_epoch{args.num_epoch}_seq{args.seq_len}_lr{args.learning_rate}_klg{args.klg_use}'
    ...
    with tqdm(total=train_steps) as t:
        for epoch in range(args.num_epoch):
            ...
            for batch in train_loader:
                ...
                """Output loss."""
                # set_trace()
                input_ids, token_ids, pos_ids, label_ids, _, loss_mask = get_batch(batch, tokenizer, training=True, seq_len=args.seq_len, device=args.device)
                losses, logits, _ = model(input_ids=input_ids, token_type_ids=token_ids, position_ids=pos_ids, labels=label_ids, return_dict=False)
            ...
            with open(os.path.join(save_path, "epoch" + str(epoch)) + ".json",'w', encoding='utf-8') as f:
                with torch.no_grad():
                    for batch in valid_loader:
                        input_ids, token_ids, pos_ids, label_ids, klg_ids, loss_mask = get_batch(batch, tokenizer, device=args.device)                        
